...
在规则卡片右侧点「编辑」或「删除」。删除时会弹出确认框，确认后才会真正删除。
...
如果你之前用的是早期版本（数据存在 KV 存储里），首次启动新版插件时会**自动从 KV 迁移一次**，把旧规则写入 `rules.json`。迁移完成后不再依赖 KV。
...
1. 停掉 AstrBot
2. 用 SQLite 工具打开数据目录下的 `data_v2.db`
3. 执行：
   ```sql
...
4. 从 `value` 里复制出 JSON 数组
5. 启动 AstrBot，进入规则管理页，粘贴到 JSON 框，点「导入（覆盖）」
...
1. 修改 `main.py`、`index.html` 等代码文件
2. 修改 `metadata.yaml` 的 `version`、`desc` 等字段（**不要改 `name`**）
3. 重载插件
...
1. 查看 AstrBot 日志中是否出现 `[要素察觉] 加载完成`。
2. 确认规则是否正确保存：进入插件页面看规则卡片是否存在。
3. 确认触发消息是**群消息**，且包含纯文本。
...
旧版本使用了浏览器原生的 `confirm()`，但 AstrBot 的插件页面运行在 sandboxed iframe 中，原生弹窗被禁用。新版本已改为自定义确认弹窗，如果还遇到问题，请更新到最新版本并刷新页面。
